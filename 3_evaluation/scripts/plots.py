...
def kruskal_dunn(df, group_col, score_col) -> pd.DataFrame:
    """
    Kruskal-Wallis omnibus test + Dunn pairwise post-hoc (Bonferroni correction).
    Returns a DataFrame of pairwise results; prints the omnibus statistic.
    """
    groups = sorted(df[group_col].unique())
    samples = [df[df[group_col] == g][score_col].dropna().values for g in groups]

    h, p_omnibus = stats.kruskal(*samples)
    print(f"Kruskal-Wallis  H={h:.3f}  p={p_omnibus:.4f}  ({group_col} → {score_col})")

    # Dunn (1964) pairwise post-hoc with Bonferroni correction
    n = sum(len(s) for s in samples)
    ranks = stats.rankdata(np.concatenate(samples))

    # tie-correction term: sum(t_j^3 - t_j) over all tie groups
    _, counts = np.unique(ranks, return_counts=True)
    tie_term = np.sum(counts ** 3 - counts)

    # pooled variance with tie correction (Dunn 1964, eq. 1)
    sigma2 = (n * (n + 1) / 12) - tie_term / (12 * (n - 1))

    offset, group_ranks = 0, {}
    for g, s in zip(groups, samples):
        group_ranks[g] = ranks[offset: offset + len(s)]
        offset += len(s)

    pairs = list(combinations(groups, 2))
    rows  = []
    for a, b in pairs:
        ra, rb = group_ranks[a], group_ranks[b]
        se = np.sqrt(sigma2 * (1 / len(ra) + 1 / len(rb)))
        z  = (ra.mean() - rb.mean()) / se if se > 0 else np.nan
        p_raw = 2 * stats.norm.sf(abs(z)) if np.isfinite(z) else np.nan
        p_bon = min(p_raw * len(pairs), 1.0) if np.isfinite(p_raw) else np.nan
        rows.append({"group_a": label(a), "group_b": label(b),
                     "z": round(z, 3), "p_raw": round(p_raw, 4),
                     "p_bonferroni": round(p_bon, 4),
                     "sig": "***" if p_bon < 0.001 else "**" if p_bon < 0.01
                             else "*" if p_bon < 0.05 else "ns"})
